...
file_dir <- "/Users/drewcolbert/Documents/Syracuse/Term 5/IST 719/Final Poster/kaggle_gross_rent.csv"
housing <- read.csv(file_dir, 
...
zap2 <- which(housing_clean$State_ab == "HI")
...
map(database = "state",
    ...
    border = "black",
    bg = "white")
my.cols <- rep(rgb(1, 1, 1, alpha = 0.3), length(us.cities$name))
...
#########################################################################
df5 <- aggregate(housing_clean$Median,
                 list(housing_clean$County, housing_clean$state),
                 mean)
...
df5$county <- gsub(" parish", "", df5$county)
...
income <- read.csv("/Users/drewcolbert/Documents/Syracuse/Term 5/IST 719/Final Poster/Unemployment.csv",
                     header = TRUE,
                     stringsAsFactors = FALSE)
# only look at the counties and the median houshold income
income <- income[,c(2,3,4,88)]

# change the column names
colnames(income) <- c("state", "stabr", "county", "median_income")

# delete alaska and hawaii
income <- income[-which(income$state == "alaska"),]
income <- income[-which(income$state == "hawaii"),]

# rename the counties to match the map() names
income$county <- tolower(income$county)
tmp <- gsub(" county", "", income$county)
tmp <- gsub("[,].*", "", tmp)
tmp <- gsub("[/].*", "", tmp)
income$county <- tmp

# tennesse and louisiana had some inconsistancies that need to be fixed
income$county[which(income$stabr == "TN")] <- gsub(" ", "", income$county[which(income$stabr == "TN")])
income$county[which(income$stabr == "LA")] <- gsub(" parish", "", income$county[which(income$stabr == "LA")])
for (i in 1:dim(income)[1]) {
    income$county[i] = paste(income$state[i], income$county[i], sep = ",")
}

# change the income column to numeric
income$median_income <- gsub(",", "", income$median_income)
income$median_income <- as.numeric(income$median_income)

# rescale the values and assign a color to the values
num.cols <- 11
col.vec <- rev(brewer.pal(num.cols, "RdYlGn"))
income$index <- round(rescale(x = income$median_income, c(1, num.cols)), 0)
income$color <- col.vec[income$index]

county.order3 <- match.map(database = "county",
                           regions = income$county,
                           exact = TRUE,
                           warn = TRUE)


map(database = "county",
    col = income$color[county.order3],
    fill = TRUE,
    resolution = 0,
    lty = 1,
    border = "tan")
gradientLegend(valRange = income$median_income, 
               col = rev(brewer.pal(num.cols, "RdYlGn")),
               side = 3,
               dec = 0,
               n.seg = 1,
...
income_rent <- merge(df5, income, by = "county")

plot(x = income_rent$median_income,
     y = income_rent$rent,
     type = "p",
     xlab = "Median Income",
     ylab = "Rent",
     col = "forestgreen", # rgb(255,140,0, maxColorValue = 255, alpha = 75),
     cex = .85,
     las = 1)
abline(lm(income_rent$rent ~ income_rent$median_income), 
       col = "red",
       lwd = 2)

# only run this once
# income_rent <- income_rent[-which(income_rent$rent == 0),]

#################################################################################

income_rent_new <- income_rent[, c(1,2,3,8)]
colnames(income_rent_new) <- c("county", "state", "rent", "income")

income_rent_new$rent_pct <- (income_rent_new$rent/income_rent_new$income)*100
rent_pct_col <- rep("orange", nrow(income_rent_new))
rent_pct_col[income_rent_new$rent_pct > 2.0] <- "red"
rent_pct_col[income_rent_new$rent_pct < 1.0] <- "forestgreen"
rent_pct_col[income_rent_new$state == "california"] <- "blue"
income_rent_new$rent_pct_color <- rent_pct_col

plot(x = income_rent_new$rent_pct,
     type = "p",
     col = income_rent_new$rent_pct_color,
     pch = 16)

##############################################################################

df5[which(df5$state == "california"),]
df5[which(df5$state == "new hampshire"),]


##############################################################################

hist(income_rent_new$rent,
     col = "firebrick",
     border = "firebrick",
     main = "",
     xlab = "",
     ylab = "")
hist(income_rent_new$income,
     col = "chartreuse4",
     border = "chartreuse4",
     main = "",
     xlab = "",
